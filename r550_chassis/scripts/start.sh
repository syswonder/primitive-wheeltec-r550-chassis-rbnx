...
#   3. Source chassis ROS2 workspace overlay
#   4. Configure Python paths
#   5. Start r550 chassis service
#
...
if [[ -f "$CODEGEN_SETUP" ]]; then
    echo "[r550_chassis/run] sourcing generated ROS2 interfaces"
    # shellcheck disable=SC1091
    set +u
    source "$CODEGEN_SETUP"
    set -u
else
    echo "[r550_chassis/run] warning: generated ROS2 interfaces not found:"
    echo "  $CODEGEN_SETUP"
fi
# --------------------------------------------------
...
do
    if [[ -f "$cand" ]]; then
        echo "[r550_chassis/run] sourcing ROS2 workspace:"
        echo "  $cand"
        # shellcheck disable=SC1091
        set +u
        source "$cand"
...
echo "[r550_chassis/run] starting chassis service"
exec python3 -m r550_chassis.main