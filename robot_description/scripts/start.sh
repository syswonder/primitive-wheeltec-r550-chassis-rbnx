...
#   3. Source robot description ROS2 workspace overlay
#   4. Configure Python paths
#   5. Start r550 robot description service
#
...
if [[ -f "/opt/ros/${ROS_DISTRO}/setup.bash" ]]; then
    ...
    echo "[r550_robot_description/run] error: ROS2 setup not found:"
...
if [[ -f "$CODEGEN_SETUP" ]]; then
    echo "[r550_robot_description/run] sourcing generated ROS2 interfaces"
    # shellcheck disable=SC1091
    set +u
    source "$CODEGEN_SETUP"
    set -u
else
    echo "[r550_robot_description/run] warning: generated ROS2 interfaces not found:"
    echo "  $CODEGEN_SETUP"
fi
# --------------------------------------------------
...
do
    if [[ -f "$cand" ]]; then
        echo "[r550_robot_description/run] sourcing ROS2 workspace:"
        echo "  $cand"
        # shellcheck disable=SC1091
        set +u
        source "$cand"
...
echo "[r550_robot_description/run] starting chassis service"
