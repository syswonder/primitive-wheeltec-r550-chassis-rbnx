...
#   3. Source astra_s_camera ROS2 workspace overlay
#   4. Configure Python paths
#   5. Start astra_s_camera chassis service
#
...
if [[ -f "$CODEGEN_SETUP" ]]; then
    echo "[astra_s_camera/run] sourcing generated ROS2 interfaces"
    # shellcheck disable=SC1091
    set +u
    source "$CODEGEN_SETUP"
    set -u
else
    echo "[astra_s_camera/run] warning: generated ROS2 interfaces not found:"
    echo "  $CODEGEN_SETUP"
fi
# --------------------------------------------------
...
do
    if [[ -f "$cand" ]]; then
        echo "[astra_s_camera/run] sourcing ROS2 workspace:"
        echo "  $cand"
        # shellcheck disable=SC1091
        set +u
        source "$cand"
...
echo "[astra_s_camera/run] starting astra_s_camera service"
